...
\begin{frame}{The bottleneck is getting evidence, not using it}
  \begin{columns}[c,onlytextwidth]
    \column{0.54\textwidth}
      ...
      \begin{tikzpicture}
        \begin{axis}[
            ybar, width=\textwidth, height=4.6cm, bar width=20pt,
            ymin=0, ymax=1, ylabel={\small GPT-4.1 judge},
            xtick={1,2,3},
        ...
        \addplot[fill=gray!35, draw=gray!55] coordinates {(1,0.5797) (2,0.6294)};
        \addplot[fill=accent, draw=accent] coordinates {(3,0.8668)};
        \end{axis}
      \end{tikzpicture}
      \end{center}
    \column{0.42\textwidth}
      ...
      \vspace{0.8em}
      \begin{itemize}
        \item Teacher \alert{evidence}: \;$+29$ pts.
        \item Teacher \alert{query hint}: \;$+5$ pts.
      \end{itemize}
      \vspace{0.6em}
      Handing over the evidence recovers the model; handing over the query
      strings barely moves it.
  \end{columns}
\end{frame}
